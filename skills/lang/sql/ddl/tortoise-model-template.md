...
class Referral(TimestampMixin, Model):
    ...
    def __str__(self) -> str:
        return f"<Referral {self.invite_code} {self.inviter_session_id} -> {self.invitee_session_id}>"
```
...
| `ForeignKeyField("models.User", related_name="orders")` | INT + FK |

## Meta 选项

```python
class Meta:
    table = "snake_case_table_name"      # 显式表名
...
- [ ] 所有 Model 继承 `TimestampMixin` + `Model`？
- [ ] `Meta.table` 显式声明 snake_case？
- [ ] 索引字段加 `index=True`？
- [ ] 外键明确 `on_delete` 行为？
...
- 父：[`./index.md`](./index.md)
- 兄弟：[`alembic-migration-template.md`](./alembic-migration-template.md)
